...
class Statistics:
    ...
    def profit_results():
        ...
        for m in p.NODES:
            ...
            #if p.model==1:
            #    Statistics.profits[i][6]= m.uncles
            #    Statistics.profits[i][7]= round(m.stales/Statistics.staleBlocks * 100,2)
            #else: Statistics.profits[i][6]=0; Statistics.profits[i][7]=0
            #Statistics.profits[i][8]= m.balance
    ...
    def global_chain():
        if p.model==0 or p.model==1:
                for i in c.global_chain:
                    total_block_reward = p.Breward + Incentives.transactions_fee(i)
                    block= [i.depth, i.id, i.previous, i.timestamp, i.miner, len(i.transactions), i.size, total_block_reward, i.weight]
    ...
    def print_to_excel(fname):
        ...
        df1 = pd.DataFrame({'Block Time': [p.Binterval], 'No. Miners': [len(p.NODES)], 'Simulation Time': [p.simTime]})
        ...
        df3.columns = ['Miner ID', '% Hash Power','# Mined Blocks', '% of main blocks','# Uncle Blocks','% of uncles','Profit']
        ...
        else: df4.columns= ['Block Depth', 'Block ID', 'Previous Block', 'Block Timestamp', 'Miner ID', '# transactions', 'Block Size', 'Total Block Reward', 'Block Weight']
