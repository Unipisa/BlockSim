...
class BlockCommit(BaseBlockCommit):
    ...
    def generate_block (event):
        ...
        minerId = miner.id
        IsSegWitNode = miner.isSegWitNode
        ...
        if blockPrev == miner.last_block().id:
            ...
            if p.hasTrans:
                if p.Ttechnique == "Light": blockTrans,blockSize,blockWeight,blockPropTime = LT.execute_transactions(IsSegWitNode)
                elif p.Ttechnique == "Full": blockTrans,blockSize,blockWeight,blockPropTime = FT.execute_transactions(miner,eventTime,IsSegWitNode)

                event.block.transactions = blockTrans
                event.block.size= blockSize
                event.block.weight= blockWeight
            ...
            BlockCommit.propagate_block(event.block,blockPropTime)
    ...
    def propagate_block (block, block_prop_time):
        #blockDelay= Network.block_prop_delay(block_prop_time) # draw block propagation delay from a distribution !! or you can assign 0 to ignore block propagation delay
        for recipient in p.NODES:
            if recipient.id != block.miner:
                mean_block_prop_time = np.log(2) / block_prop_time
                quantile_probability = ((recipient.id + 1) - 0.5) / p.Nn
                quantile = (-1 * np.log(1 - quantile_probability)) / mean_block_prop_time
                #blockDelay= Network.block_prop_delay(block_prop_time) # draw block propagation delay from a distribution !! or you can assign 0 to ignore block propagation delay
                Scheduler.receive_block_event(recipient,block,quantile)
