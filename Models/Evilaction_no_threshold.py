...
class LightTransaction():
    ...
    def reference_block_statistics(filename, transactions_list):
    ...
    def execute_transactions(simulation_folder_reference_txs_timestamp, run_id, block_count, minerId, is_evil_block):
        ...
        if (run_id % 2 == 0) or (run_id % 2 != 0 and is_evil_block == False):
            ...
            LightTransaction.reference_block_statistics(stats_filename, transactions)
        ...
        while count < len(pool):
            ...
            for i in range(len(pool)):
                #print("ENTRO!!!")
                if str(i) in selected_tx_indexes:
                    continue
                if (block_weight >= pool[i].weight):
                    expected_block_reward_new = LightTransaction.expected_reward(block_reward_from_fees + pool[i].fee, size + pool[i].size)
                    #print("\nexpected_block_reward_new: {}".format(expected_block_reward_new))
                    #print("expected_block_reward_current: {}".format(expected_block_reward_current))
            ...
            if found == False:
                break
            selected_tx_indexes[str(selected_tx_index)] = True
            block_weight -= pool[i].weight
            transactions += [pool[i]]
            size += pool[i].size
            weight += pool[i].weight
            if pool[i].weight < 4 * pool[i].size:
                count_segwit+=1
                #print ("\nCOUNT: {} - SEGWIT".format((count_segwit)))
            else:
                count_legacy+=1
                #print ("\nCOUNT: {} - LEGACY".format((count_legacy)))
            block_reward_from_fees += pool[i].fee
