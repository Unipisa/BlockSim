        ...
        while count < len(transactions):
            ...
            for i in range(len(transactions)):
                ...
                if str(i) in selected_tx_indexes:
                    continue
                if (block_weight >= transactions[i].weight):
                    expected_block_reward_new = LightTransaction.expected_reward(block_reward_from_fees + transactions[i].fee, evil_block_size + transactions[i].size)
            ...
            block_weight -= transactions[i].weight
            evil_transactions += [transactions[i]]
            evil_block_size += transactions[i].size
            evil_block_weight += transactions[i].weight
            if transactions[count].weight < 4 * transactions[count].size:
                ...
                count_legacy+=1
                #print ("\nCOUNT: {} - LEGACY".format((count_legacy)))
            block_reward_from_fees += transactions[i].fee
